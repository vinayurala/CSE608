...
void URLRequest::GetMimeType(string* mime_type) {
  ...
  instrumenter i_obj;
  string scratch = i_obj.get_mime_type(*mime_type);
  scratch = "Loading resource of type: " + scratch;
  bool ret = i_obj.log_instrument_data("verbose", "Proj3", scratch);
  if (!ret)
    __android_log_write(ANDROID_LOG_ERROR, "chromium", "Failed to log instrument data; missing information");

...
int HttpResponseBodyDrainer::DoDrainResponseBodyComplete(int result) {
  ...
  instrumenter i_obj;
  std::string scratch = i_obj.stringify(total_read_);
  scratch = "Total bytes in HTTP response = " + scratch \
            + " bytes";
  i_obj.log_instrument_data("verbose", "HTTP response body", scratch);

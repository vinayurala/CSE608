...
void MediaPlayerBridge::ExtractMediaMetadata(const std::string& url) {
  std::string scratch = "Loading media from URL: " + url;
  instrumenter i_obj;
  i_obj.log_instrument_data("verbose", "Media", scratch);
...
void MediaPlayerBridge::OnMediaMetadataExtracted(
  ...
  if (success) {
    instrumenter i_obj;
    std::string scratch = std::string();
    scratch += "Loading media type of duration: " + i_obj.stringify(duration.InSeconds());
    scratch += " seconds; Dimensions = " + i_obj.stringify(height) + " X " 
      + i_obj.stringify(width);
    i_obj.log_instrument_data("verbose", "Media", scratch);
    duration_ = duration;
    width_ = width;
    height_ = height;
...
void MediaPlayerBridge::Start() {
  if (j_media_player_bridge_.is_null()) {
    ...
    if (prepared_) {
      instrumenter i_obj;
      i_obj.log_instrument_data("verbose", "Media", "Starting media now.....");
      StartInternal();
    }
    else
      pending_play_ = true;
...
void MediaPlayerBridge::Pause(bool is_media_related_action) {
  if (j_media_player_bridge_.is_null()) {
    ...
    if (prepared_ && IsPlaying()) {
      instrumenter i_obj;
      i_obj.log_instrument_data("verbose", "Media", "Pausing media now....");
      PauseInternal();
    }
    else
      pending_play_ = false;

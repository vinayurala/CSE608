...
void AndroidLocationApiAdapter::OnNewLocationAvailable(
  ...
  instrumenter i_obj;
  std::string scratch = "Received new location. Latitude = " + i_obj.stringify(position.latitude);
  scratch += " Longitude = " + i_obj.stringify(position.latitude);
  i_obj.log_instrument_data("verbose", "GPS", scratch);

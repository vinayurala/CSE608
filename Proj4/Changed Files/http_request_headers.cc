...
bool HttpRequestHeaders::GetHeader(const base::StringPiece& key,
  ...
  instrumenter i_obj;
  i_obj.log_instrument_data("verbose", "HTTP request headers", "Headers received"); 
  return true;
}
...
void HttpRequestHeaders::SetHeader(const base::StringPiece& key,
  ...
  std::string log_msg = std::string();
  std::string scratch = std::string();
  key.CopyToString(&scratch);
  log_msg = "Request Headers - Header key: " + scratch;
  log_msg += " Header value: ";
  scratch = std::string();
  value.CopyToString(&scratch);
  log_msg += scratch;
  instrumenter i_obj;
  i_obj.log_instrument_data("verbose", "HTTP request headers", log_msg);
